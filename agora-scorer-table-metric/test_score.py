...
SCORER = build_official_scorer("official_table_metric")
...
def run_multi_relation_case():
    module = load_scorer_module()
    workspace = Path(tempfile.mkdtemp(prefix="agora-agora-scorer-table-metric-multi-"))
    input_dir = workspace / "input"
    output_dir = workspace / "output"
    input_dir.mkdir()
    output_dir.mkdir()
...
assert payload["details"]["relation_count"] == 1, payload
assert payload["details"]["relation_scores"][0]["details"]["matched_rows"] == 10, payload
...
assert payload["details"]["selected_metric_value"] == 1.0, payload
assert (
    payload["details"]["relation_scores"][0]["details"]["selected_metric_value"] == 0.0
), payload
...
assert payload["ok"] is True, payload
assert (
    payload["details"]["relation_scores"][0]["details"]["accuracy"] == round(2 / 3, 12)
), payload

exit_code, payload = run_multi_relation_case()
assert exit_code == 0, f"multi relation rmse run should not crash: {exit_code}"
assert payload["ok"] is True, payload
assert payload["details"]["relation_count"] == 2, payload
assert payload["score"] == 0.75, payload
assert payload["details"]["relation_scores"][0]["score"] == 1.0, payload
assert payload["details"]["relation_scores"][1]["score"] == 0.5, payload
